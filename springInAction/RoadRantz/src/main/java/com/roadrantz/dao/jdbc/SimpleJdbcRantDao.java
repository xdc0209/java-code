...
public class SimpleJdbcRantDao extends SimpleJdbcDaoSupport implements RantDao
{
    ...
    {
        getSimpleJdbcTemplate().update(RANT_INSERT, rant.getRantText(), rant.getVehicle().getId(),
                rant.getPostedDate());
    ...
    {
        return getSimpleJdbcTemplate().query(RANT_SELECT, new ParameterizedRowMapper<Rant>()
        {
            public Rant mapRow(ResultSet rs, int rowNum) throws SQLException
            {
                Rant rant = new Rant();
    ...
    {
        return getSimpleJdbcTemplate().queryForObject(VEHICLE_BY_ID_SELECT, new ParameterizedRowMapper<Vehicle>()
        {
    ...
    {
        List<Vehicle> matches = getSimpleJdbcTemplate().query(VEHICLE_BY_PLATE_SELECT,
    ...
    {
        getSimpleJdbcTemplate().update(VEHICLE_INSERT, vehicle.getPlateNumber(), vehicle.getState());
        vehicle.setId(queryForIdentity());
    }

    public Motorist getMotoristByEmail(String email)
    {
        List<Motorist> matches = getSimpleJdbcTemplate().query(MOTORIST_BY_EMAIL_SELECT,
    ...
    {
        List<Motorist> matches = getSimpleJdbcTemplate().query(MOTORIST_BY_ID_SELECT,
                new ParameterizedRowMapper<Motorist>()
                {
                    public Motorist mapRow(ResultSet rs, int rowNum) throws SQLException
                    {
    ...
    {
        getSimpleJdbcTemplate().update(MOTORIST_INSERT, motorist.getEmail(), motorist.getPassword(),
