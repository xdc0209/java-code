...
public class JdbcRantDao extends JdbcDaoSupport implements RantDao
{
    ...
    private static final String RANT_FOR_VEHICLE_SELECT  = RANT_SELECT + " where vehicle_id = ?";

    private static final String RANT_FOR_DAY_SELECT      = RANT_SELECT + " where postedDate=?";

    private static final String VEHICLE_SELECT           = "select id, plateNumber, state from vehicle";

    private static final String VEHICLE_BY_ID_SELECT     = VEHICLE_SELECT + " where id=?";

    private static final String VEHICLE_BY_PLATE_SELECT  = VEHICLE_SELECT + " where state=? and plateNumber=?";
    ...
    private static final String MOTORIST_BY_EMAIL_SELECT = MOTORIST_SELECT + " where email=?";
    ...
    public void saveRant(Rant rant)
    {
        getJdbcTemplate().update(RANT_INSERT,
                new Object[] { rant.getRantText(), rant.getVehicle().getId(), rant.getPostedDate() });
    ...
    {
        Integer motoristId = vehicle.getMotorist() != null ? vehicle.getMotorist().getId() : null;
        getJdbcTemplate().update(VEHICLE_INSERT,
                new Object[] { vehicle.getPlateNumber(), vehicle.getState(), motoristId });
    ...
    {
        return getJdbcTemplate().query(RANT_FOR_DAY_SELECT, new Object[] { day }, new RowMapper()
        {
            public Object mapRow(ResultSet rs, int rowNum) throws SQLException
            {
                Rant rant = new Rant();
    ...
    {
        List matches = getJdbcTemplate().query(VEHICLE_BY_ID_SELECT, new Object[] { id }, new RowMapper()
        {
    ...
    {
        List matches = getJdbcTemplate().query(MOTORIST_BY_ID_SELECT, new Object[] { Long.valueOf(id) }, new RowMapper()
        {
            public Object mapRow(ResultSet rs, int rowNum) throws SQLException, DataAccessException
            {
    ...
    {
        List matches = getJdbcTemplate().query(VEHICLE_BY_PLATE_SELECT, new Object[] { state, plateNumber },
                new RowMapper()
                {
                    public Object mapRow(ResultSet rs, int rowNum) throws SQLException, DataAccessException
                    {
        ...
        List rants = getJdbcTemplate().query(RANT_FOR_VEHICLE_SELECT, new Object[] { vehicle.getId() }, new RowMapper()
        {
            public Object mapRow(ResultSet rs, int rowNum) throws SQLException
            {
                Rant rant = new Rant();
    ...
    {
        List matches = getJdbcTemplate().query(MOTORIST_BY_EMAIL_SELECT, new Object[] { email }, new RowMapper()
        {
            public Object mapRow(ResultSet rs, int rowNum) throws SQLException, DataAccessException
            {
                Motorist motorist = new Motorist();
