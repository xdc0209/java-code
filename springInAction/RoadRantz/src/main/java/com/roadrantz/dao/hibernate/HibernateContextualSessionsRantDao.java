...
public class HibernateContextualSessionsRantDao implements RantDao
{
    ...
    {
        List results = sessionFactory.getCurrentSession().find(
                "from " + VEHICLE + " where state = ? and plateNumber = ?", new Object[] { state, plateNumber },
    ...
    {
        List results = sessionFactory.getCurrentSession().find("from " + MOTORIST + " where email = ?",
                new Object[] { email }, new Type[] { Hibernate.STRING });
    ...
    {
        return sessionFactory.getCurrentSession().find("from " + RANT + " where postedDate = ?", day, Hibernate.DATE);
    }
    ...
    {
        return (Motorist) sessionFactory.getCurrentSession().load(Motorist.class, id);
    }
