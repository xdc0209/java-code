...
public class RantsForVehicleControllerTest extends TestCase
{
    ...
    public void testSimpleCase() throws Exception
    {
        ...
        assertEquals("View name should be 'vehicleRants'", "vehicleRants", modelAndView.getViewName());
        ...
        assertEquals("Model element 'rants' should contain 2 items", 2, rants.size());
        for (Iterator iter = rants.iterator(); iter.hasNext();)
        {
            Rant rant = (Rant) iter.next();
            assertEquals("Rant's Vehicle state is incorrect", TEST_STATE, rant.getVehicle().getState());
            assertEquals("Rant's Vehicle plateNumber is incorrect", TEST_PLATE_NUMBER,
