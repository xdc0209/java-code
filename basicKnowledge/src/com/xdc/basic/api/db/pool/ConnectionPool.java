...
public class ConnectionPool
{
    ...
    private void createConnections(int numConnections) throws SQLException
    {
        ...
        for (int x = 0; x < numConnections; x++)
        {
            ...
            // 假如 maxConnections为 0或负数，表示连接数量没有限制。
            // 假如连接数己经达到最大，即退出。
    ...
    private Connection newConnection() throws SQLException
    {
        ...
        if (connections.size() == 0)
        {
            ...
            // 数据库返回的 driverMaxConnections若为 0，表示此数据库没有最大连接限制，或数据库的最大连接限制不知道
            // driverMaxConnections为返回的一个整数，表示此数据库答应客户连接的数目
            // 假如连接池中设置的最大连接数量大于数据库答应的连接数目 , 则置连接池的最大连接数目为数据库答应的最大数目
    ...
    public synchronized Connection getConnection() throws SQLException
    {
        ...
        while (conn == null)
        {
            // 等一会再试
            wait(250);
            // 重新再试，直到获得可用的连接，假如getFreeConnection()返回的为 null，则表明创建一批连接后也不可获得可用连接
    ...
    private Connection findFreeConnection() throws SQLException
    {
        ...
        // 获得连接池向量中所有的对象。遍历所有的对象，看是否有可用的连接
        ...
        while (enumerate.hasMoreElements())
        {
            ...
            if (!pConn.isBusy())
            {
                ...
                {
                    // 假如此连接不可再用了，则创建一个新的连接，并替换此不可用的连接对象，假如创建失败，返回 null
                    try
                    {
                        conn = newConnection();
                    }
    ...
    private boolean testConnection(Connection conn)
    {
        try
        {
            ...
            if (testTable.equals(""))
            {
                // 假如测试表为空，试着使用此连接的 setAutoCommit()方法来判定连接否可用。
                // 此方法只在部分数据库可用，假如不可用，抛出异常。
                // 注意：使用测试表的方法更可靠。
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            // do something...
            Statement stmt = conn.createStatement();
            stmt.execute("select count(*) from dual");
            stmt.getResultSet();
