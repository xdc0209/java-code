...
import com.xdc.basic.commons.time.DateFormatUtil;
...
public class ScheduledThreadPoolExecutorTest
{
    ...
    private static void cancelSchedule(ScheduledExecutorService exec, long initialDelay, long period)
    {
        ScheduledFuture<?> scheduledFuture = exec.scheduleAtFixedRate(new Runnable()
        {
            ...
            public void run()
            {
                System.out.println("cancelSchedule: " + DateFormatUtil.format(new Date(), DATE_FORMAT));
            }
        ...
        System.out.println(
                "cancelSchedule: the schedule has been canceled!" + DateFormatUtil.format(new Date(), DATE_FORMAT));
    }
    ...
    private static void scheduleAtFixedRate(ScheduledExecutorService exec, long initialDelay, long period)
    {
        ...
        exec.scheduleAtFixedRate(new Runnable()
        {
            ...
            public void run()
            {
                System.out.println("scheduleAtFixedRate: " + DateFormatUtil.format(new Date(), DATE_FORMAT));
            }
        }, initialDelay, period, TimeUnit.MILLISECONDS);
    }
    ...
    private static void scheduleWithFixedDelay(ScheduledExecutorService exec, long initialDelay, long period)
    {
        ...
        exec.scheduleWithFixedDelay(new Runnable()
        {
            ...
            public void run()
            {
                System.out.println("scheduleWithFixedDelay-begin: " + DateFormatUtil.format(new Date(), DATE_FORMAT));
                try
                {
                    Thread.sleep(2000);
                }
                ...
                System.out.println("scheduleWithFixedDelay-end: " + DateFormatUtil.format(new Date(), DATE_FORMAT));
            }
        }, initialDelay, period, TimeUnit.MILLISECONDS);
    }
