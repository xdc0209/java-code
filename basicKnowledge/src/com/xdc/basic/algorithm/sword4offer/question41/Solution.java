...
{
    private BinarySearchTree<Integer, Integer> tree = new BinarySearchTree<Integer, Integer>();
    ...
    public Double GetMedian()
    {
        ...
        if ((tree.size() & 1) != 0)
        {
            return (double) tree.select((tree.size() + 1) >> 1);
        }
        else
        {
            return (tree.select(tree.size() >> 1) + tree.select((tree.size() >> 1) + 1)) / 2.0;
        }
    }
